...
do
    ...
    do
        for NUM_GPUS in "${NUM_GPUS_ITER[@]}"
        do
            # Set other dependent variables, if any
            NUM_GPUS_PERNODE=$NUM_GPUS  # Example, adjust if necessary
            ...
            NAME="gpt-${MODEL_SIZE}B-bs-${BATCH_SIZE}-gpus-${NUM_GPUS}-mp-${MP_SIZE}-ep-${EP_SIZE}"
            if [[ $EP_SIZE -gt 1 ]]; then
                # NAME="${NAME}-ep-${EP_SIZE}-mlc-${MLC}-cap-${MOE_TRAIN_CAP_FACTOR}-drop-${MOE_DROP_TOKEN}"
                NAME="${NAME}-epp-${EP_PARALLEL_SIZE}"
            fi
            ...
            #Jing added --skip-train for only inference
            megatron_options=" \
                    --skip-train \
                    --inference \
