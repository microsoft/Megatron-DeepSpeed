...
cl_metric_2nd="seqlen_truncate"
cl_cluster_type_2nd="single_cluster"
cl_start_2nd=128
cl_end_2nd=${seq_len}
cl_step_in_million_2nd=0.96
cl_root_degree_2nd=1
cl_step_2nd=$(calc $cl_step_in_million*1000000)
###############################################################################
### Misc configs
log_interval=100
eval_iters=10
...
jobname="${jobname}-${cl_metric_2nd}-from${cl_start_2nd}-to${cl_end_2nd}-step${cl_step_in_million_2nd}M-root${cl_root_degree_2nd}"
...
template_json="ds_config_bert_data_efficiency_2metric_TEMPLATE.json"
...
sed "s/CONFIG_BATCH_SIZE/${global_batch_size}/" ${template_json} \
    ...
    | sed "s#CL_1st_METRIC_NAME#${cl_metric}#" \
    | sed "s#CL_1st_CLUSTER_TYPE#${cl_cluster_type}#" \
    | sed "s#CONFIG_CL_1st_SAMPLE_PATH#${index_to_sample_path}#" \
    | sed "s#CONFIG_CL_1st_METRIC_PATH#${index_to_metric_path}#" \
    | sed "s/CONFIG_CL_1st_MIN/${cl_start}/" \
    | sed "s/CONFIG_CL_1st_MAX/${cl_end}/" \
    | sed "s/CONFIG_CL_1st_DURATION/${cl_step}/" \
    | sed "s/CONFIG_CL_1st_ROOT_DEGREE/${cl_root_degree}/" \
    | sed "s#CL_2nd_METRIC_NAME#${cl_metric_2nd}#" \
    | sed "s#CL_2nd_CLUSTER_TYPE#${cl_cluster_type_2nd}#" \
    | sed "s/CONFIG_CL_2nd_MIN/${cl_start_2nd}/" \
    | sed "s/CONFIG_CL_2nd_MAX/${cl_end_2nd}/" \
    | sed "s/CONFIG_CL_2nd_DURATION/${cl_step_2nd}/" \
    | sed "s/CONFIG_CL_2nd_ROOT_DEGREE/${cl_root_degree_2nd}/" \
      > ${config_json}
else
sed "s/CONFIG_BATCH_SIZE/${global_batch_size}/" ${template_json} \
    | sed "s/CONFIG_MBSIZE/${batch_size}/" \
