...
WARMUP_TOKENS=3000000000 #375000000
LR_DECAY_TOKENS=$(( ${TRAIN_TOKENS} - ${WARMUP_TOKENS} ))

INIT_SEQ_LEN=2048
INCRESE_LEN_TOKEN_INTERVAL=1.75
###############################################################################
### Parallelism configs
...
host="${HOSTNAME}"
NAME="gpt3-${MODEL_SIZE}B-tokendropping-graduallyIncrease--${LR}-${TRAIN_TOKENS}-${INIT_SEQ_LEN}-${INCRESE_LEN_TOKEN_INTERVAL}"
# NAME="gpt3-${MODEL_SIZE}B-Baseline-longerwarmup3b-useTokenLayer"
# if [ "${NO_PP}" = "true" ]; then
#     NAME="${NAME}-no_pp"
# fi
...
CHECKPOINT_PATH="/blob/users/zheweiyao/project/tokendropping/checkpoint_v2/${NAME}"
mkdir -p ${LOG_PATH}
...
DATA_PATH=/tmp/the_pile_public_merged_nopreprocessing/pile_text_document
...
megatron_options=" \
        ...
        --num-workers 0 \
        --fp16 \
        --initial-sequence-length ${INIT_SEQ_LEN} \
        --increse-length-token-interval ${INCRESE_LEN_TOKEN_INTERVAL} \
...
run_cmd="deepspeed --num_nodes 8 --num_gpus 8 ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${LOG_PATH}/${NAME}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
