...
host="${HOSTNAME}"
NAME="gpt3-${MODEL_SIZE}B-tokendropping-graduallyIncrease-lr-${LR}"
# if [ "${NO_PP}" = "true" ]; then
#     NAME="${NAME}-no_pp"
# fi
...
CHECKPOINT_PATH="/blob/users/xiaoxiawu/project/tokendropping/checkpoint/${NAME}"
mkdir -p ${LOG_PATH}
...
template_json="configs/ds_config_gpt_0.35B_TEMPLATE.json"
config_json="configs/ds_config_${NAME}.json"
...
run_cmd="deepspeed ${DIR}/../../pretrain_gpt.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${LOG_PATH}/${NAME}_${current_time}.log"
echo ${run_cmd}
eval ${run_cmd}
set +x
