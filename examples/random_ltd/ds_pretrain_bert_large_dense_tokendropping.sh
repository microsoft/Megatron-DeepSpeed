...
min_lr=1e-5
MASTER_PORT=6000$1
dropout=$2 ## init_std is the standard deviation for weight initialization. Usually larger
...
# TRAIN_TOKENS=30000000000 ####=30,000,000,000 
...
batch_size=$(( ${global_batch_size} / ${dp_size}/2))
###############################################################################
### Misc configs
log_interval=100
eval_iters=10
...
jobname="${jobname}-lr-${lr}-min-${min_lr}-wmup-${lr_warmup_iters}-dcy-0.${lr_decay_iters_in_million}M-sty-${lr_decay_style}"
...
log_parent_dir="./"
log_path="${log_parent_dir}/log_test/"
## Microsoft internal constraint: because tensorboard is logged by last rank,
## it's better to put the path in NFS instead of Blob.
tensorboard_dir="${log_parent_dir}/tensorboard_test/"
...
data_options=" \
    ...
    --data-path ${data_path} \
    --data-impl mmap"
#    --mask-prob 0.075 --short-seq-prob 0.05 \
megatron_options=" --random-ltd \
    ...
    --fp16 --attention-dropout ${dropout} --hidden-dropout ${dropout} \
...
template_json="configs/ds_config_bert_TEMPLATE.json"
config_json="configs/ds_config_${jobname}.json"
...
sed "s/CONFIG_BATCH_SIZE/${global_batch_size}/" ${template_json} \
    ...
    | sed "s/CONFIG_RANDOMLTD_ENABLED/true/" \
      > ${config_json}
else
sed "s/CONFIG_BATCH_SIZE/${global_batch_size}/" ${template_json} \
    | sed "s/CONFIG_MBSIZE/${batch_size}/" \
...
run_cmd="deepspeed  ${dir}/../../pretrain_bert.py ${megatron_options} ${data_options} ${deepspeed_options} &> ${log_path}/${jobname}_${host}.log" # _${current_time}
echo ${run_cmd}
eval ${run_cmd}
set +x
