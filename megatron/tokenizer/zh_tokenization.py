...
def load_vocab(dict_path, encoding='utf-8', simplified=False, startswith=None):
    ...
    if simplified:  # 过滤冗余部分token
        ...
        for t, _ in sorted(token_dict.items(), key=lambda s: s[1]):
            if t not in new_token_dict and not ZHBertTokenizer._is_redundant(t):
                new_token_dict[t] = len(new_token_dict)
                keep_tokens.append(token_dict[t])
...
class ZHBertTokenizer(TokenizerBase):
    """Bert原生分词器
    ...
    def __init__(
        ...
        for token in ['pad', 'unk', 'mask', 'start', 'end']:
            try:
                _token_id = vocab[getattr(self, '_token_%s' % token)]
    ...
    def token_to_id(self, token):
        """token转换为对应的id
        """
        return self.vocab.get(token, self._token_unk_id)

    def id_to_token(self, i):
        """id转换为对应的token
        """
        return self.inv_vocab[i]
    ...
    def _is_redundant(token):
        ...
        """
        if len(token) > 1:
            for ch in ZHBertTokenizer.stem(token):
                if (
                        ZHBertTokenizer._is_cjk_character(ch) or
                        ZHBertTokenizer._is_punctuation(ch)
                ):
                    return True
