...
class CheckpointFunction(torch.autograd.Function):
    ...
    """
    @staticmethod
    def forward(ctx, run_function, sequence_length, *args):
        ...
        with torch.no_grad():
            if sequence_length < 2048:
                ...
                attention_masks = args[1]
                tmp_2 = args[2]
                tmp_3 = args[3]

                rand_list = []
                part_hidden_states = [] # seq x batch x hidden 4kx4k * 1kx4k (32kx4k) 
                for i in range(hidden_states.size(1)):
                    B_tmp = torch.randperm(hidden_states.size(0), device=hidden_states.device)[:sequence_length]
                    B = B_tmp.sort()[0]
                    rand_list.append(B)
                    part_hidden_states.append(hidden_states[B, i:i+1, :])
                    print(B)
                part_hidden_states = torch.cat(part_hidden_states, dim=1)
                part_attention_mask = attention_masks[:, :, :sequence_length, :sequence_length]
                part_hidden_states, moe_loss = run_function(part_hidden_states, part_attention_mask, tmp_2, tmp_3)
                for i in range(hidden_states.size(1)):
                    hidden_states[rand_list[i], i, :].data.copy_(part_hidden_states[:, i, :].data)
    ...
    def backward(ctx, *args):
        ...
        with torch.enable_grad():
            if sequence_length < 2048:
                hidden_states = detached_inputs[0]
                attention_masks = detached_inputs[1]
                tmp_2 = detached_inputs[2]
                tmp_3 = detached_inputs[3]
                ...
                part_hidden_states = torch.cat(part_hidden_states, dim=1)
                part_attention_mask = attention_masks[:, :, :sequence_length, :sequence_length]
                # part_hidden_states, moe_loss = ctx.run_function(part_hidden_states, part_attention_mask, tmp_2, tmp_3)
                outputs, moe_loss = ctx.run_function(part_hidden_states, part_attention_mask, tmp_2, tmp_3)
                # outputs = ctx.run_function(part_hidden_states, part_attention_mask, tmp_2, tmp_3)
                # import pdb; pdb.set_trace()
                for i in range(hidden_states.size(1)):
                    hidden_states[rand_list[i], i, :] = part_hidden_states[:, i, :]
                # outputs, _ = (hidden_states, moe_loss)
                # import pdb; pdb.set_trace()
            else:
        ...
        if sequence_length < 2048:
            # import pdb; pdb.set_trace()
            part_grad_output = []
            for i in range(grad_output.size(1)):
                part_grad_output.append(grad_output[rand_list[i], i:i+1, :])
            part_grad_output = torch.cat(part_grad_output, dim=1)
            torch.autograd.backward(outputs, part_grad_output)
        else:
            torch.autograd.backward(outputs, grad_output)
        grads = [inp.grad if isinstance(inp, torch.Tensor) else inp
                      for inp in detached_inputs]
        import pdb; pdb.set_trace()
        # this part is a bit tricky for our case, based on my understanding, we shall write the gradients back to the original tensor
        if sequence_length < 2048:
            for i in range(grad_output.size(1)):
                grad_output[rand_list[i], i:i+1, :] = grads[0][rand_list[i], i:i+1, :]
            grads[0] = grad_output
